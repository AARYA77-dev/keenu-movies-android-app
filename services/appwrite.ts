...
export const updateSearchCount = async (query: string, movie: Movie) => {
    try {
        ...
        if (result.documents.length > 0) {
            ...
            await database.createDocument(DATABASE_ID, COLLECTION_ID, ID.unique(), { searchTerm: query, movie_id: movie.id, count: 1, poster_url: `https://image.tmdb.org/t/p/w500${movie.poster_path}`, title: movie.title })
        }
    } catch (err) {
        console.log(err);
        throw err;
